...
Nexus introduces a novel dual architecture via the NexusEVM and NexusCore, two parallel state machines replicated by NexusBFT, a custom consensus protocol optimized for the dual architecture. Together, the execution and consensus layers are co-designed to achieve high-throughput and low-latency, well suited for HFTs and end-users who seek to migrate tradidional trading strategies onchain.

The Nexus dual block architecture allows purpose-built engines, called co-processors, to be enshrined natively at the execution layer, giving EVM smart contracts access to accelerated native functionality traditionally unavailable in standard blockchain architectures at the expense of more complex harware requirements for node operators, thus requiring higher compute and memory. 

Nexus begins by enshrining a high-performance spot and perpetuals central limit order book (CLOB) DEX as the first financial engine in NexusCore, giving NexusEVM smart contracts the ability to create their own high-frequency markets on top of a high-performance orderbook engine. As the system progresses, Nexus seeks to enshrine other functionality such as high-frequency oracles, prediction and information markets, vaults, lending, bridging, native stablecoin gas and more.

Our vision for the Nexus Exchange is to showcase the full breadth of programmability and performance possible via NexusCore and the co-processor model, and to bring to life a credibly-neutral decentralized Internet exchange that gives applications and users access to high liquidity, depth and onchain volume. We believe enshrinement of core financial engines may allow the $10T+/day market of traditional financial trading volume across perps, spot, FX, equities, treasuries and beyond onchain, to the hands of worldwide users and developers, within a fully programmable, composable and permissionless shared computer.
...
The long-term zero-knowledge vision of Nexus would bring significant vertical and horizontal scalability to the validation capabilities of the blockchain and would democratize the validation process -- for example, even phones could validate and communicate with the blockchain directly without intermediaries -- without sacrificing performance. Central to this long-term vision is the Nexus zkVM, the verifiable compute engine core to our vision from Day 1, see the original [Nexus Whitepaper](https://whitepaper.nexus.xyz).
...
Traditional blockchain architectures force a choice between generality and performance. Ethereum's EVM delivers comprehensive programmability but lacks the latency and throughput needed for high-frequency financial applications. Application-specific blockchains achieve superior performance but sacrifice composability and ecosystem interoperability.

Nexus addresses this architectural limitation through a **co-processor model** that unifies general-purpose programmability with specialized high-performance execution. This architecture enables new financial applications while expanding DeFi programmability and creating new revenue capture mechanisms directly within the Layer-1 protocol.

### The Nexus Innovation: Layered Architecture with Dual Execution

#### Three-Layer System Design

Nexus implements a **three-layer architecture** that separates concerns for optimal performance, verification, and consensus:

**Execution Layer** (Transaction Processing):
- **NexusEVM**: Ethereum-compatible virtual machine for general smart contracts
- **NexusCore**: High-performance co-processors for specialized financial operations (CLOB DEX, with planned support for Lending, Staking, Vaults, Oracles, LSTs, RWAs, Bridges, and stablecoins)

**Verification Layer** (Proof Generation):
- **Nexus zkVM**: Verifiable compute engine for end-to-end execution proving
- **Nexus Network**: Distributed compute network scaling Layer-1 validation

**Consensus Layer** (Network Agreement):
- **NexusBFT**: Custom consensus protocol optimized for dual execution and co-processor governance

This separation enables independent optimization of each layer while maintaining global consistency and long-term scalability through zero-knowledge verification.
...
Within the execution layer, Nexus operates **two parallel execution environments**, each optimized for different computational requirements:
...
- **Use Cases**: DeFi protocols, NFT systems, governance contracts, complex application logic

**NexusCore** (Financial Primitives):
- **Purpose**: High-performance financial operations via specialized co-processors
- **Characteristics**: Optimized data structures, deterministic operations, <100ms latency
- **Use Cases**: Perpetuals trading, derivatives, risk management, liquidations, lending, payments, market making
...
This dual execution model addresses the performance-programmability trade-off by providing **specialized execution paths** while maintaining **unified state consistency** and atomic transaction guarantees.

### NexusCore: Enshrined Financial Co-processors

#### Co-processor Architecture Overview

Co-processors are specialized execution engines integrated directly into the NexusCore protocol. Unlike smart contracts that execute within virtual machines, co-processors operate as native protocol components with optimized implementations and dedicated resources.
...
![Co-processor State Machine Model](./images/coprocessor.png)
...
### Dual-Block Architecture: Optimized for High-Frequency Operations

Nexus employs a **dual-block architecture** that separates execution frequencies to optimize performance for different operation types:

**[GRAPHIC 5: Dual-Block Architecture]**
![Dual Block Architecture](./images/dual-block.png)

**Architecture Design**:
- **NexusCore blocks**: Continuous high-frequency execution (every 50-100ms) for co-processor operations
- **NexusEVM blocks**: Lower frequency execution (every 4-10 Core blocks) for general smart contract operations
- **Predictable scheduling**: EVM blocks occur at deterministic intervals, enabling reliable cross-domain coordination

**Performance Characteristics**:
- **Core latency**: Consistent <100ms confirmation for perpetual trading, order matching, and risk management
...
This architecture enables predictable, high-frequency operations while maintaining full EVM composability.
...
- **Direct protocol access** with <100ms execution times
...
**Performance Without Compromise**: CEX-level performance (<100ms Core, sub-second EVM) with full smart contract composability and decentralized execution
...
|------------|-----------------|---------------------|----------|
| **Performance** | 15-100 TPS, 12s+ latency | 1,000+ TPS, <1s latency | 25,000+ TPS, 200,000+ orders/s, <100ms Core, <1s EVM |
...
#### Parallel Execution & Hardware Scaling
...
|-----------|--------|------------|---------------|
| **Block Time** | $T_{\text{Block}}$ | Fixed time between all Nexus blocks | 50ms - 100ms |
...
|------------------|-------------------|--------------|-----------------|-----------------|
| 4 | 100ms | 100ms | 250ms | 400ms |
| 5 | 100ms | 100ms | 300ms | 500ms |
| 8 | 100ms | 100ms | 450ms | 800ms |
| 10 | 100ms | 100ms | 550ms | 1.0s |
...
|----------|------------------------------|--------------|-----------------|-----------|
| **HFT-Focused** | 8-10 | 100ms | 800ms-1.0s | Minimize Core latency |
| **Balanced** | 4-5 | 100ms | 400ms-500ms | Balance both use cases |
| **EVM-Heavy** | 2-3 | 100ms | 200ms-300ms | Minimize EVM latency |
